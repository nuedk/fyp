...
from gui_agent.utils.common import call_llm_safe, split_thinking_response
...
class CodeExecutor:
    ...
    def __init__(self, engine_params: Dict, budget: int = 20):
        ...
        logger.info(f"CodeExecutor initialized with budget={budget}")
        self.reset()

    def reset(self):
        """Reset the code executor state."""
        logger.debug("Resetting CodeExecutor state")
        self.agent = LMMAgent(
            engine_params=self.engine_params,
            system_prompt=PROCEDURAL_MEMORY.CODE_EXECUTOR_PROMPT,
        )
