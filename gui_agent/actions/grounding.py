...
from gui_agent.memory.procedural import PROCEDURAL_MEMORY
from gui_agent.core.mllm import LMMAgent
from gui_agent.utils.common import call_llm_safe
from gui_agent.actions.code_executor import CodeExecutor
import logging
...
class OSWorldACI(ACI):
    def __init__(
        ...
        width: int = 1920,
        height: int = 1080,
        code_executor_budget: int = 20,
        code_executor_engine_params: Dict = None,
    ):
        super().__init__()
        ...
        code_executor_engine_params = (
            code_executor_engine_params or engine_params_for_generation
        )
        self.code_executor = CodeExecutor(code_executor_engine_params, code_executor_budget)

        # Store task instruction for code executor
        self.current_task_instruction = None
        self.last_code_executor_result = None
    ...
    def set_task_instruction(self, task_instruction: str):
        """Set the current task instruction for the code executor."""
    ...
    @agent_action
    def call_code_executor(self, task: str = None):
        """Call the code executor to execute code for tasks or subtasks that can be completed solely with coding.
        ...
        **🚨 CRITICAL GUIDELINES:**
        ...
        """
        logger.info("=" * 50)
        logger.info("GROUNDING AGENT: Calling code executor")
        logger.info("=" * 50)
        ...
        if task_to_execute:
            ...
            self.last_code_executor_result = result

            logger.info("code executor execution completed")
            logger.info(f"Result - Completion reason: {result['completion_reason']}")
            logger.info(f"Steps executed: {result['steps_executed']}")
            logger.info(f"Summary: {result['summary']}")

            logger.info("=" * 50)
            logger.info("GROUNDING AGENT: code executor Call Finished")
            logger.info("=" * 50)

            # Return code to be executed in the environment
            return "import time; time.sleep(2.222)"
        else:
            logger.warning("No task instruction available for code executor call")
