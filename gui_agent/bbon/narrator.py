...
from gui_agent.utils.common import (
    call_llm_formatted,
    split_thinking_response,
    compress_image,
)
from gui_agent.utils.formatters import (
...
class Narrator:
    ...
    def judge(
        ...
        mouse_actions = Narrator.extract_mouse_action(pyautogui_action)
        before_img = Image.open(BytesIO(before_img_bytes))
        Narrator.mark_action(mouse_actions, before_img)
        out_buffer = BytesIO()
        ...
        fact_message = [
            {
                "role": "system",
                "content": PROCEDURAL_MEMORY.NARRATOR_SYSTEM_PROMPT,
            }
        ]
